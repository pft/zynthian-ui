...
class zynthian_gui_base(tkinter.Frame):
    ...
    def init_dpmeter(self):
        ...
        self.dpm_a = zynthian_gui_dpm(self.zyngui.state_manager.zynmixer, 0, True, 0, self.status_canvas, 0, 0, width, height, False, ("status_dpm"))
        self.dpm_b = zynthian_gui_dpm(self.zyngui.state_manager.zynmixer, 0, True, 1, self.status_canvas, 0, height + 2, width, height, False, ("status_dpm"))

    def refresh_status(self):
        if self.shown:
            mute = self.zyngui.state_manager.zynmixer.get_mute(0, True)
            ...
            if not mute and self.dpm_a:
                state = self.zyngui.state_manager.zynmixer.get_dpm_states(0, 0, True)[0]
